...
def local_data_generator(width, length, bn):
    ...
    for i in range(0, bn):
        ...
        params[i,:,4]=sortedd.loc[:, 'parameter-0':'parameter-'+str(sortedd.shape[1]-3)].iloc[i]-boundaries[3].values*width
        params[i,:,5]=sortedd.loc[:, 'parameter-0':'parameter-'+str(sortedd.shape[1]-3)].iloc[i]+boundaries[3].values*width
